  ...
  return (
    <div className="page">
      ...
      <div className="emp-header-bar">
        ...
        <button onClick={() => nav(`/org/${slug}/submit`)} style={{ background: 'var(--brand)', border: 'none', borderRadius: 8, padding: '6px 12px', cursor: 'pointer', fontSize: 12, fontWeight: 700, color: 'white', fontFamily: 'inherit' }}>✅ المهام</button>
      </div>
      ...
      <div className="submit-bar">
        <button className="btn-primary" onClick={handleSubmit} disabled={submitting} style={{ background: 'var(--ink)' }}>
    ...
    <div>
      {categories.map(cat => (
        <div key={cat} style={{ marginBottom: 24 }}>
          <div style={{ background: 'var(--ink)', color: 'white', padding: '10px 16px', fontWeight: 800, fontSize: 14 }}>{cat}</div>
                ...
                {items.filter(i => i.category === cat).map((item, idx) => (
                  <tr key={item.id} style={{ background: idx % 2 === 0 ? 'white' : '#f8fafc', borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={{ ...TD, padding: '4px 6px', textAlign: 'center' }}>
                      {readOnly
                        ? <span style={{ fontWeight: 800, color: entries?.[item.id]?.qty ? 'var(--ink)' : 'var(--muted)' }}>
                            {entries?.[item.id]?.qty || '-'}
                          </span>
                        : <input type="number" min="0" placeholder="0"
                            style={{ width: 65, padding: '5px 6px', border: '1.5px solid var(--border)', borderRadius: 6, fontSize: 13, textAlign: 'center', fontFamily: 'inherit', outline: 'none' }}
