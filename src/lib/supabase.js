...
export async function signupAdmin({ adminName, username, password }) {
  ...
  const { data, error } = await supabase.from('admins').insert({ name: adminName, username, password }).select().single()
  if (error) throw error
  return data
}

export async function loginAdmin({ username, password }) {
  const { data } = await supabase.from('admins').select('*').eq('username', username).eq('password', password).maybeSingle()
...
export async function getOrgsByAdmin(adminId) {
  const { data } = await supabase.from('orgs').select('*').eq('admin_id', adminId).order('created_at', { ascending: false })
  return data || []
}

export async function createOrg({ adminId, orgName, slug }) {
  const safeSlug = slug.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')
  const { data: existing } = await supabase.from('orgs').select('id').eq('slug', safeSlug).maybeSingle()
  if (existing) throw new Error('هذا الرابط مستخدم، جرب اسماً آخر')
  const { data, error } = await supabase.from('orgs').insert({ admin_id: adminId, name: orgName, slug: safeSlug }).select().single()
  if (error) throw error
...
export async function addSection({ orgId, name, icon }) {
  const { data, error } = await supabase.from('sections').insert({ org_id: orgId, name, icon: icon || '🏢' }).select().single()
  if (error) throw error
  return data
}
...
export async function getEmployees(orgId) {
  const { data } = await supabase.from('employees').select('*, sections(name, icon), employee_sections(section_id, sections(id, name, icon))').eq('org_id', orgId).order('name')
  return data || []
}

export async function addEmployee({ orgId, name, pin, sectionId, canOrders }) {
  const { data: existing } = await supabase.from('employees').select('id').eq('org_id', orgId).eq('pin', pin).maybeSingle()
  if (existing) throw new Error('هذا الكود مستخدم بالفعل')
  const { data, error } = await supabase.from('employees').insert({ org_id: orgId, name, pin, section_id: sectionId || null, can_orders: canOrders || false }).select().single()
  if (error) throw error
  return data
}
...
export async function getEmployeeSections(employeeId) {
  const { data } = await supabase.from('employee_sections').select('section_id').eq('employee_id', employeeId)
  return (data || []).map(r => r.section_id)
}

export async function setEmployeeSections(employeeId, sectionIds) {
  // Delete all existing
  await supabase.from('employee_sections').delete().eq('employee_id', employeeId)
  // Insert new ones
  if (sectionIds.length > 0) {
    const rows = sectionIds.map(sid => ({ employee_id: employeeId, section_id: sid }))
    const { error } = await supabase.from('employee_sections').insert(rows)
    if (error) throw error
  }
}

export async function updateEmployee(id, { sectionId, canOrders }) {
  const { error } = await supabase.from('employees')
    .update({ section_id: sectionId || null, can_orders: canOrders || false })
    .eq('id', id)
  if (error) throw error
}

export async function loginEmployee({ orgId, pin }) {
  const { data } = await supabase.from('employees').select('*, sections(name, icon), employee_sections(section_id, sections(id, name, icon))').eq('org_id', orgId).eq('pin', pin).maybeSingle()
  if (!data) throw new Error('الكود غير صحيح')
  return data
}

// ─── TASKS ────────────────────────────────────────────────────────────────────
export async function getTasks(orgId, sectionId = null) {
  let q = supabase.from('tasks').select('*').eq('org_id', orgId).order('category').order('created_at')
  if (Array.isArray(sectionId) && sectionId.length > 0) {
    q = q.in('section_id', sectionId)
  } else if (sectionId && !Array.isArray(sectionId)) {
...
export async function addTask({ orgId, category, name, sectionId }) {
  const { data, error } = await supabase.from('tasks').insert({ org_id: orgId, category, name, section_id: sectionId || null }).select().single()
  if (error) throw error
  return data
}
...
export async function updateTaskFull(id, name, sectionId) {
  const { error } = await supabase.from('tasks')
    .update({ name, section_id: sectionId || null })
    .eq('id', id)
  if (error) throw error
}
...
export async function getSubmissions(orgId, { employeeId, date } = {}) {
  let q = supabase.from('submissions').select('*').eq('org_id', orgId).order('submitted_at', { ascending: false })
...
export async function getTodaySubmission(orgId, employeeId) {
  ...
  const { data } = await supabase.from('submissions').select('*').eq('org_id', orgId).eq('employee_id', employeeId).eq('date', today).maybeSingle()
  return data || null
}
...
export async function getOrderItems(orgId) {
  const { data } = await supabase.from('order_items').select('*').eq('org_id', orgId).order('category').order('created_at')
  return data || []
}

export async function addOrderItem({ orgId, category, name, nameEn, nameAr, itemNo, code, uom }) {
  const { data, error } = await supabase.from('order_items').insert({ org_id: orgId, category, name, name_en: nameEn || name, name_ar: nameAr || '', item_no: itemNo || '', code: code || '', uom: uom || '' }).select().single()
  if (error) throw error
  return data
}
...
export async function getOrderSubmissions(orgId, { employeeId, date } = {}) {
  let q = supabase.from('order_submissions').select('*').eq('org_id', orgId).order('submitted_at', { ascending: false })
  if (employeeId) q = q.eq('employee_id', employeeId)
  if (date) q = q.eq('date', date)
  const { data } = await q
  return data || []
...
export async function getTodayOrderSubmission(orgId, employeeId) {
  const today = new Date().toISOString().split('T')[0]
  const { data } = await supabase.from('order_submissions').select('*').eq('org_id', orgId).eq('employee_id', employeeId).eq('date', today).maybeSingle()
  return data || null
}
...
export async function seedOrderItems(orgId) {
  ...
  const rows = DEFAULT_ORDER_ITEMS.map(t => ({
    org_id: orgId, category: t.category,
...
export async function uploadPhoto(orgId, employeeId, taskId, file) {
  ...
  if (!exists) throw new Error('يرجى إنشاء bucket باسم task-photos في Supabase Storage')
