...
export async function getEmployees(orgId) {
  const { data } = await supabase.from('employees').select('*, sections(name, icon)').eq('org_id', orgId).order('name')
  return data || []
}
...
export async function loginEmployee({ orgId, pin }) {
  const { data } = await supabase.from('employees').select('*, sections(name, icon)').eq('org_id', orgId).eq('pin', pin).maybeSingle()
