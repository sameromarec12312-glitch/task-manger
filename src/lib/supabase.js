...
export async function addOrderItem({ orgId, category, name }) {
  const { data, error } = await supabase
    .from('order_items').insert({ org_id: orgId, category, name }).select().single()
  if (error) throw error
  return data
}
...
export async function seedOrderItems(orgId) {
  ...
  const rows = DEFAULT_ORDER_ITEMS.map(t => ({ org_id: orgId, ...t }))
