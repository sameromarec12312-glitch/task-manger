...
import { getTasks, addTask, updateTask, deleteTask, getEmployees, addEmployee, deleteEmployee, getSubmissions, deleteSubmission, getSections, addSection, deleteSection } from '../lib/supabase'
...
export default function AdminDashboard() {
  ...
  const loadAll = async (orgId) => {
    setLoading(true)
    const [t, e, s, sec] = await Promise.all([getTasks(orgId), getEmployees(orgId), getSubmissions(orgId), getSections(orgId)])
  ...
  return (
    <div className="page">
      ...
      <div className="tabs">
        {[['reports','📊 التقارير'],['tasks','📋 المهام'],['employees','👥 الموظفين'],['sections','📍 الأقسام']].map(([k,l]) => (
