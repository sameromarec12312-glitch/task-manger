                                ...
                                {categories.map(cat => {
                                  ...
                                  return (
                                    <div key={cat} style={{ marginBottom: 10 }}>
                                      <div className="cat-label">{cat}</div>
                                      <table className="data-table">
                                        <thead><tr><th>الصنف</th><th style={{ textAlign: 'center' }}>الكمية</th><th>ملاحظة</th></tr></thead>
                                        <tbody>{filled.map(i => {
                                          const e = sub.entries[i.id]
                                          return (
                                            <tr key={i.id}>
                                              ...
                                              <td className="num">{e?.qty || '-'}</td>
                                              <td style={{ fontSize: 11, color: 'var(--muted)', fontStyle: 'italic' }}>{e?.note || '-'}</td>
                                            </tr>
                                          )
                                        })}</tbody>
                                      </table>
        ...
        <>
          {orderItems.length === 0 && (
            <div style={{ marginBottom: 16 }}>
              <button className="btn-primary" onClick={handleSeedItems} disabled={seeding}
              ...
              <p style={{ fontSize: 12, color: 'var(--muted)', marginTop: 6, textAlign: 'center' }}>
                كل الأصناف من قائمتك ستضاف دفعة واحدة
              </p>
            </div>
          )}

          <div className="card">
            <h3 style={{ fontWeight: 800, fontSize: 15, marginBottom: 12 }}>إضافة صنف جديد</h3>
            <div className="form-grid">
              ...
              <input className="input-plain" placeholder="اسم الصنف" value={newName} onChange={e => setNewName(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddItem()} />
                ...
                <div key={i.id} className="task-admin-row">
                  {editId === i.id
                    ...
                    : <><span style={{ fontSize: 13 }}>{i.name}</span>
                      <button className="icon-btn" onClick={() => { setEditId(i.id); setEditName(i.name) }}>✏️</button>
