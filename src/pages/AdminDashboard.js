    ...
    <div className="page">
      <nav className="topnav">
        <div>
          ...
          <div className="topnav-sub" onClick={backToOrgs} style={{ cursor:"pointer", color:"rgba(255,255,255,0.5)", fontSize:11 }}>← العودة لقائمة المؤسسات</div>
        </div>
        <button className="topnav-logout" onClick={logout}>خروج</button>
      </nav>

      <div style={{ background: "var(--brand-light)", padding: "10px 16px", borderBottom: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 700, margin: '0 auto' }}>
          <p style={{ fontSize: 12, color: "var(--brand-dark)", fontWeight: 700, marginBottom: 4 }}>🔗 رابط موظفي هذه المؤسسة:</p>
...
function ReportsTab({ submissions, employees, tasks, sections, reload }) {
  ...
  const buildCSV = (subs, empLabel) => {
    ...
    const infoRows = [
      ['تقرير أداء الموظفين - Original Shawarma'],
      [`الموظف: ${empLabel}`],
      [`الفترة: ${dateRange}`],
    ...
    sorted.forEach(sub => {
      ...
      categories.forEach(cat => {
        ...
        catTasks.forEach(t => {
          ...
          dataRows.push([firstRow ? sub.date : '', firstRow ? dayName : '', firstRow ? sub.employee_name : '', firstRow ? (sub.section_name || '-') : '', cat, t.name, e?.qty||'', e?.time||'', e?.note||''])
          firstRow = false
        })
      })
      if (dataRows.length) dataRows.push([])
    })
    const summaryRows = [[],['ملخص'],['إجمالي التسليمات', subs.length],['عدد الأيام', new Set(subs.map(s => s.date)).size]]
    return [...infoRows, ...dataRows, ...summaryRows]
  }

  const exportCSV = () => {
    const empName = filterEmp ? employees.find(e => e.id === filterEmp)?.name || 'موظف' : 'كل الموظفين'
    const rows = buildCSV(filtered, empName)
    const csv = rows.map(r => r.map(v => `"${String(v).replace(/"/g,'""')}"`).join(',')).join('\n')
    const blob = new Blob(['\uFEFF'+csv], {type:'text/csv;charset=utf-8'})
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob)
    a.download = `تقرير_${empName}_${filterFrom||'كل'}.csv`; a.click()
  }

  const exportAllEmployeesCSV = () => {
    employees.forEach(emp => {
      const empSubs = filtered.filter(s => s.employee_id === emp.id)
      if (!empSubs.length) return
      const rows = buildCSV(empSubs, emp.name)
      const csv = rows.map(r => r.map(v => `"${String(v).replace(/"/g,'""')}"`).join(',')).join('\n')
      const blob = new Blob(['\uFEFF'+csv], {type:'text/csv;charset=utf-8'})
      const a = document.createElement('a'); a.href = URL.createObjectURL(blob)
      a.download = `تقرير_${emp.name}_${filterFrom||'كل'}.csv`; a.click()
    })
  }
  ...
  const downloadSub = (sub) => {
    ...
    categories.forEach(cat => {
      ...
      filled.forEach(t => {
        const e = sub.entries[t.id]
        dataRows.push([cat, t.name, e?.qty||'', e?.time||'', e?.note||''])
      })
    })
    const rows = [...infoRows, ...dataRows]
    const csv = rows.map(r => r.map(v => `"${String(v).replace(/"/g,'""')}"`).join(',')).join('\n')
    const blob = new Blob(['\uFEFF'+csv], {type:'text/csv;charset=utf-8'})
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob)
      ...
      <div style={{ marginBottom: 6, borderRadius: 8, overflow: 'hidden', border: '1px solid var(--border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', background: 'white' }}>
          <button onClick={() => setOpenSub(isOpen ? null : sub.id)} style={{
            ...
            <div style={{ width: 34, height: 34, borderRadius: '50%', background: 'var(--brand)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 13, color: 'white', flexShrink: 0 }}>
              {sub.employee_name[0]}
            </div>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2, flexWrap: 'wrap' }}>
                ...
                {sub.section_name && (
                  <span style={{ fontSize: 10, background: 'var(--brand-light)', color: 'var(--brand-dark)', padding: '1px 8px', borderRadius: 20, fontWeight: 700 }}>
          ...
          <button onClick={() => downloadSub(sub)} style={{ padding: '8px 10px', background: 'none', border: 'none', cursor: 'pointer', fontSize: 15, color: 'var(--green)' }} title="تحميل هذا اليوم">⬇</button>
          <button onClick={() => handleDelete(sub.id)} disabled={isDel} style={{ padding: '8px 10px', background: 'none', border: 'none', cursor: 'pointer', fontSize: 16, color: 'var(--red)', opacity: isDel ? 0.5 : 1 }}>
            {isDel ? '...' : '🗑'}
          </button>
        </div>
        {isOpen && (
          <div style={{ background: '#fafafa', padding: '10px 12px' }}>
            {sub.section_name && (
              <div style={{ background: 'var(--brand-light)', borderRadius: 8, padding: '6px 12px', marginBottom: 10, fontSize: 13, fontWeight: 700, color: 'var(--brand-dark)' }}>
                    ...
                    <tbody>{filled.map(t => {
                      ...
                      return (
                        <tr key={t.id}>
                          ...
                          <td style={{ padding: '4px 8px' }}>{e?.photo
                            ? <img src={e.photo} alt="task" style={{ width: 60, height: 60, objectFit: 'cover', borderRadius: 6, cursor: 'pointer' }}
  ...
  const GroupHeader = ({ icon, title, count, groupKey }) => {
    ...
    return (
      <button onClick={() => setOpenGroup(isOpen ? null : groupKey)} style={{
        ...
        padding: '12px 16px', background: 'var(--ink)', border: 'none', cursor: 'pointer', fontFamily: "'Cairo', sans-serif"
      }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ background: 'var(--brand)', color: 'white', padding: '2px 10px', borderRadius: 20, fontSize: 12, fontWeight: 700 }}>{count} تسليم</span>
    ...
    <div className="tab-content">
      ...
      <div className='card' style={{ marginBottom: 12 }}>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          <button className="btn-sm btn-green" onClick={exportCSV} title="تصدير CSV">⬇ CSV</button>
          <button className="btn-sm btn-blue" onClick={exportAllEmployeesCSV} title="تصدير ملف منفصل لكل موظف">⬇ كل الموظفين</button>
        </div>
        ...
        {hasFilters && (
          <button onClick={clearFilters} className='btn-sm btn-red' style={{ marginTop: 8 }}>✕ مسح الفلاتر</button>
        )}
      </div>
      ...
      {viewMode === 'employee' && Object.entries(byEmployee).map(([empId, { name, subs }]) => {
        ...
        return (
          <div key={empId} className='card' style={{ marginBottom: 10, padding: 0, overflow: 'hidden' }}>
      ...
      {viewMode === 'section' && Object.entries(bySection).map(([secName, { icon, subs }]) => {
        ...
        return (
          <div key={secName} className='card' style={{ marginBottom: 10, padding: 0, overflow: 'hidden' }}>
...
function OrdersTab({ orderItems, orderSubmissions, employees, orgId, reload }) {
  ...
  const buildOrderCSV = (subs, empLabel) => {
    const infoRows = [
      ['تقرير طلبيات المشتريات - Original Shawarma'],
      [`الموظف: ${empLabel}`],
      [`تاريخ التصدير: ${new Date().toLocaleDateString('ar-SA')}`],
      [],
      ['الموظف','التاريخ','الفئة','Item No.','Code','UoM','Description','Arabic','QTY'],
    ]
    const dataRows = []
    const sorted = [...subs].sort((a, b) => b.date.localeCompare(a.date))
    sorted.forEach(sub => {
    ...
    return [...infoRows, ...dataRows]
  }

  const exportCSV = () => {
    const empName = filterEmp ? employees.find(e => e.id === filterEmp)?.name || 'موظف' : 'كل الموظفين'
    const rows = buildOrderCSV(filtered, empName)
    const csv = rows.map(r => r.map(v => `"${String(v).replace(/"/g, '""')}"`).join(',')).join('\n')
    const blob = new Blob(['\uFEFF' + csv], { type: 'text/csv;charset=utf-8' })
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob)
    a.download = `طلبيات_${empName}_${filterFrom||'كل'}.csv`; a.click()
  }
  ...
  const exportAllOrdersCSV = () => {
    employees.forEach(emp => {
      const empSubs = filtered.filter(s => s.employee_id === emp.id)
      if (!empSubs.length) return
      const rows = buildOrderCSV(empSubs, emp.name)
      const csv = rows.map(r => r.map(v => `"${String(v).replace(/"/g, '""')}"`).join(',')).join('\n')
      const blob = new Blob(['\uFEFF' + csv], { type: 'text/csv;charset=utf-8' })
      const a = document.createElement('a'); a.href = URL.createObjectURL(blob)
      a.download = `طلبيات_${emp.name}.csv`; a.click()
    })
  }
        ...
        <>
          <div className='card' style={{ marginBottom: 12 }}>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              ...
              <button className="btn-sm btn-amber" onClick={handleRefresh} disabled={refreshing}>{refreshing ? '...' : '🔄'}</button>
              <button className="btn-sm btn-green" onClick={exportCSV} title="تصدير CSV">⬇ CSV</button>
              <button className="btn-sm btn-blue" onClick={exportAllOrdersCSV} title="تصدير ملف منفصل لكل موظف">⬇ كل الموظفين</button>
            </div>
            <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
              <div style={{ flex: 1 }}><div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 3 }}>من</div><input type="date" className="input-plain" value={filterFrom} onChange={e => setFilterFrom(e.target.value)} /></div>
              <div style={{ flex: 1 }}><div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 3 }}>إلى</div><input type="date" className="input-plain" value={filterTo} onChange={e => setFilterTo(e.target.value)} /></div>
              ...
              return (
                <div key={date} className='card' style={{ marginBottom: 10, padding: 0, overflow: 'hidden' }}>
                  <button onClick={() => setOpenGroup(isOpen ? null : date)} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <span style={{ background: 'var(--brand)', color: 'white', padding: '2px 10px', borderRadius: 20, fontSize: 12, fontWeight: 700 }}>{subs.length} طلب</span>
                      <span style={{ color: 'white', fontSize: 16, transform: isOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>⌄</span>
                    </div>
                  </button>
                  {isOpen && (
                          ...
                          <div key={sub.id} style={{ marginBottom: 6, borderRadius: 8, overflow: 'hidden', border: '1px solid var(--border)' }}>
                            <div style={{ display: 'flex', alignItems: 'center', background: 'white' }}>
                              <button onClick={() => setOpenSub(isSubOpen ? null : sub.id)} style={{
                                ...
                                <div style={{ width: 34, height: 34, borderRadius: '50%', background: 'var(--ink)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 13, color: 'white', flexShrink: 0 }}>{sub.employee_name[0]}</div>
                                <div style={{ flex: 1 }}>
                              ...
                              <button onClick={() => {
                                const infoRows = [['تقرير طلبيات - Original Shawarma'],[`الموظف: ${sub.employee_name}`],[`التاريخ: ${sub.date}`],[],['Item No.','Code','UoM','Description','Arabic','QTY']]
                                const dataRows = []
                                categories.forEach(cat => { orderItems.filter(i => i.category===cat && sub.entries?.[i.id]?.qty).forEach(i => { const e=sub.entries[i.id]; dataRows.push([i.item_no||'',i.code||'',i.uom||'',i.name_en||i.name,i.name_ar||'',e?.qty||'']) }) })
                                const csv = [...infoRows,...dataRows].map(r=>r.map(v=>`"${String(v).replace(/"/g,'""')}"`).join(',')).join('\n')
                                const blob=new Blob(['\uFEFF'+csv],{type:'text/csv;charset=utf-8'})
                              ...
                              }} style={{ padding: '8px 10px', background: 'none', border: 'none', cursor: 'pointer', fontSize: 15, color: 'var(--green)' }} title="تحميل هذا الطلب">⬇</button>
                              <button onClick={() => handleDelete(sub.id)} disabled={isDel} style={{ padding: '8px 10px', background: 'none', border: 'none', cursor: 'pointer', fontSize: 16, color: 'var(--red)', opacity: isDel ? 0.5 : 1 }}>
                                {isDel ? '...' : '🗑'}
                              </button>
                            </div>
        ...
        <>
          ...
          {categories.map(cat => (
            <div key={cat} style={{ marginBottom: 20 }}>
              <div style={{ background: 'var(--ink)', color: 'white', padding: '10px 16px', fontWeight: 800, fontSize: 13 }}>{cat}</div>
              <div style={{ overflowX: 'auto' }}>
