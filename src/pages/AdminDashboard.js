    ...
    <div className="page">
      <nav className="topnav">
        <div>
          ...
          <div className="topnav-sub" style={{ cursor: 'pointer', textDecoration: 'underline' }} onClick={backToOrgs}>
...
function ReportsTab({ submissions, employees, tasks, reload }) {
  ...
  const exportCSV = () => {
    ...
    a.download = `تقرير_${filterDate || 'كل'}.csv`; a.click()
  }
        ...
        : filtered.map(sub => {
          ...
          return (
            <div key={sub.id} style={{ marginBottom: 10, borderRadius: 12, overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.07)' }}>
              <button onClick={() => setOpenId(isOpen ? null : sub.id)} style={{
                width: '100%', display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px',
                background: 'white', border: 'none', cursor: 'pointer', textAlign: 'right',
                fontFamily: "'Cairo', sans-serif", borderBottom: isOpen ? '2px solid var(--amber)' : 'none'
              }}>
                <div style={{
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>
                    📅 {sub.date} &nbsp;·&nbsp; ✅ {countFilled(sub)} مهمة
                  </div>
                </div>
                <div style={{ fontSize: 18, color: 'var(--muted)', transform: isOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>⌄</div>
              </button>
              {isOpen && (
