                ...
                <div key={task.id} style={{ background: 'white', borderBottom: '1px solid var(--border)' }}>
                  ...
                  {noteIsOpen && (
                    <div style={{ padding: '8px 14px 10px', background: 'var(--amber-light)', borderTop: '1px solid var(--amber)' }}>
                      <label style={{ fontSize: 11, color: 'var(--amber-dark)', fontWeight: 700, display: 'block', marginBottom: 4 }}>📝 ملاحظة</label>
...
function PhotoBlock({ taskId, empId, orgId, photo, uploading, onPhoto, onUploading }) {
  const handleFile = async (e) => {
    ...
    onUploading(true)
    try {
      const url = await uploadPhoto(orgId, empId, taskId, compressed)
      onPhoto(url)
    } catch (err) {
...
// Compress image to max 800px and 80% quality before upload
