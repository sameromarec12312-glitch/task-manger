...
import { addEmployee, deleteEmployee, updateEmployee } from '../lib/supabase'
...
export default function EmployeesTab({ employees, sections, orgId, appUrl, orgSlug, reload }) {
  ...
  const [editingEmp, setEditingEmp] = useState(null) // {id, name, pin, sectionId, canOrders}
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await updateEmployee(editingEmp.id, { sectionId: editingEmp.sectionId, canOrders: editingEmp.canOrders })
      await reload()
      setEditingEmp(null)
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4, flexWrap: 'wrap' }}>
                    {sec ? <span className="badge badge-brand">{sec.icon} {sec.name}</span> : <span className="badge" style={{ background: 'var(--surface-2)', color: 'var(--muted)' }}>بدون قسم</span>}
                    {e.can_orders && <span className="badge badge-green">📦 طلبيات</span>}
                    <span className="emp-code">🔑 {e.pin}</span>
                  </div>
                </div>
                <button className="btn-sm btn-blue" onClick={() => setEditingEmp({ id: e.id, name: e.name, sectionId: e.section_id || '', canOrders: e.can_orders || false })}>✏️ تعديل</button>
              ...
              {editingEmp?.id === e.id && (
                <div style={{ padding: '14px', background: 'var(--surface-2)', borderTop: '1px solid var(--border)' }}>
                  <div style={{ marginBottom: 10 }}>
                    <label className="input-label">القسم</label>
                    <select className="select-plain" value={editingEmp.sectionId}
                      onChange={ev => setEditingEmp(p => ({ ...p, sectionId: ev.target.value }))}>
